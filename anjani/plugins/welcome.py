...
class Greeting(plugin.Plugin):
    ...
    async def _member_join(self, message: Message, reply_to: Optional[int], thread_id: Optional[int]) -> None:
        chat = message.chat
                ...
                if new_member.id == self.bot.uid:
                    ...
                    async def _send_welcome(
                        *,
                        ...
                        """Send the configured welcome, with a safe fallback."""
                    ...
                    try:
                        try:
                            msg = await _send_welcome(include_context=True)
                        except (MediaEmpty, MessageEmpty):
                            raise
                        except Exception:
                            # Reply/topic context may be rejected by Telegram
                            # (especially after clean-service deletes the join
                            # message). Retry the exact same welcome as a normal
                            # message without that context.
                            self.log.exception(
                                "Contextual welcome send failed in %s; retrying without reply/topic.",
                                message.chat.id,
                            )
                            msg = await _send_welcome(include_context=False)
                    except MediaEmpty:
                        await self.bot.client.send_message(
                            message.chat.id,
                        ...
                        # Last-resort text fallback. This makes a configured
                        # welcome failure visible to members rather than losing
                        # the entire greeting silently.
                        try:
                            fallback = await self._build_text(
                                await self.text(
                                    message.chat.id, "default-welcome", noformat=True
                                ),
                                new_member,
                                chat,
                                self.bot.client,
                            ...
                            self.log.exception(
                                "Last-resort default welcome also failed in chat %s.",
                                message.chat.id,
                            )
