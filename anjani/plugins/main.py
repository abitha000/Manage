...
class Main(plugin.Plugin):
    ...
    async def cmd_start(self, ctx: command.Context) -> Optional[str]:
        ...
        if chat.type == ChatType.PRIVATE:  # only send in PM's
            ...
            # Send the start image with the existing /start text and buttons.
            # main.py is located at anjani/plugins/main.py, so this resolves
            # assets/start.jpg from the project root reliably on Koyeb/Docker.
            start_image = Path(__file__).resolve().parents[2] / "assets" / "start.jpg"
            start_text = await self.text(chat.id, "start-pm", self.bot_name)

            await self.bot.client.send_photo(
                chat_id=chat.id,
                photo=str(start_image),
                caption=start_text,
